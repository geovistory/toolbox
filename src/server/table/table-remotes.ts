...
import { GetTableOptions, GetTablePageSqlBuilder } from './sql-builders/get-table-page-sql-builder';
...
export class TableRemotes {
   ...
   * @param options
   */
  async getPage(fkProject: number, pkEntity: number, options: GetTableOptions): Promise<any> {
    ...
    if (!(options?.columns?.length >= 1)) {
      await this.getColumns(fkProject, pkEntity).then(cols => {
        options.columns = cols.dat.column.map(col => col.pk_entity)
      })
    }
