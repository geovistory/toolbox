...
#         # test with new geov app from dev (tempo app) copy db from dev
...
if [ $DB_ENV='temp' ]; then
    ...
    # delete the warehouse schema name
    ...
    heroku pg:copy geovistory-dev::DATABASE_URL DATABASE_URL -a geovistory-temp --confirm=geovistory-temp
