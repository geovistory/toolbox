...
psql $DB_URL <<EOF
    ...
    FROM (
        SELECT i.* 
        FROM information.statement i,
            projects.info_proj_rel ipr
        WHERE ipr.fk_entity = i.pk_entity 
        AND ipr.is_in_project IS TRUE
        AND ipr.fk_project = $PROJECT_ID
    ) AS item;
EOF
...
psql $DB_URL <<EOF
    ...
    SELECT DISTINCT ON (entity.pk_entity) entity.pk_entity, 0, entity.fk_class, CURRENT_TIMESTAMP 
