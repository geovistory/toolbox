...
CREATE TABLE tables."row" (
    ...
    CONSTRAINT row_fk_digital_fkey FOREIGN KEY (fk_digital) REFERENCES data.digital (pk_entity) MATCH SIMPLE ON UPDATE NO ACTION ON DELETE NO ACTION)
...
CREATE OR REPLACE FUNCTION tables.create_row_table_for_digital (pk_digital integer)
    RETURNS text
    LANGUAGE 'plpgsql'
...
ALTER TABLE tables.quill_doc_cell
    DROP CONSTRAINT quill_doc_cell_fk_row_fkey;

ALTER TABLE tables.quill_doc_cell
    ADD CONSTRAINT quill_doc_cell_fk_row_fkey FOREIGN KEY (fk_row) REFERENCES tables.row (pk_row) MATCH SIMPLE ON UPDATE NO ACTION ON DELETE NO ACTION;
