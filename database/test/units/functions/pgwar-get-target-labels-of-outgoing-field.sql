...
INSERT INTO pgwar.entity_preview (pk_entity, fk_project, fk_class, entity_label)
VALUES (33, 0, 88, 'Entity 33');

-- Test 1: Check if function returns the correct target labels for outgoing field
SELECT results_eq(
        'SELECT label::text FROM pgwar.get_target_labels_of_outgoing_field(11, 0, 22, 5)',
        ARRAY [ 'Label 1',
        'Label 2',
        'Entity 33',
        NULL,
...
-- Test 2: Check if function handles non-existing entity_id
SELECT is_empty(
        'SELECT label::text FROM pgwar.get_target_labels_of_outgoing_field(99999, 0, 22, 5)',
        'get_target_labels_of_outgoing_field returns no labels for entity_id 99999, project_id 0, property_id 22, and limit_count 5'
    );
