...
module.exports = function (ComQuery) {
    ...
    ComQuery.run = function (pkProject, query, ctx, cb) {
        ...
        const filter = {
            select: {
                include: [
                    'pk_entity', 'entity_label', 'class_label', 'entity_type', 'type_label'
                ]
            },
            where: [
                'fk_project', '=', pkProject,
                ...(query.classes.length || query.types.length ?
                    [
                        'AND',
                        [
                            ...(query.classes.length ? ['fk_class', 'IN', query.classes] : []),
                            ...(query.classes.length && query.types.length ? ['OR'] : []),
                            ...(query.types.length ? ['fk_type', 'IN', query.types] : [])
                        ]
                    ] : []
                )
            ],
