...
exports.up = function (db, callback) {
  ...
  const sql = `
  CREATE OR REPLACE FUNCTION commons._validate_json_schema_type(type text, data jsonb) RETURNS boolean AS $f$
  BEGIN
  ...
  CREATE OR REPLACE FUNCTION commons.validate_json_schema(schema jsonb, data jsonb, root_schema jsonb DEFAULT NULL) RETURNS boolean AS $f$
  DECLARE
    prop text;
    item jsonb;
    path text[];
...
exports.down = function (db, callback) {
  ...
  const sql = `
    ...
    DROP FUNCTION commons._validate_json_schema_type(text, jsonb);
    DROP FUNCTION commons.validate_json_schema(jsonb, jsonb, jsonb);
