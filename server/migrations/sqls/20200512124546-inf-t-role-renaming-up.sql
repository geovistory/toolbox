-- 1 rename information.role -> statement
Alter Table information.role Rename To statement;

-- 2 rename information.role_vt -> statement_vt
Alter Table information.role_vt Rename To statement_vt;

-- 3 rename column information.statement (fk_temporal_entity) -> (fk_subject_info)
Alter Table information.statement Rename Column fk_temporal_entity To fk_subject_info;

-- 4 rename column information.statement_vt (fk_temporal_entity) -> (fk_subject_info)
Alter Table information.statement_vt Rename Column fk_temporal_entity To fk_subject_info;

-- 5 rename column information.statement (fk_entity) -> (fk_object_info)
Alter Table information.statement Rename Column fk_entity To fk_object_info;

-- 6 rename column information.statement_vt (fk_entity) -> (fk_object_info)
Alter Table information.statement_vt Rename Column fk_entity To fk_object_info;

-- 7 drop view information.v_role
Drop View information.v_role;

-- 8 create view information.v_statement
...
-- 9 rename trigger function v_role_find_or_create -> v_statement_find_or_create()
Alter Function information.v_role_find_or_create Rename To v_statement_find_or_create;
...
Create Or Replace Function information.get_outgoing_statements_to_add (
    entity_id integer,
    project_id integer
)
    Returns Table (
...
--  fn-3 DROP function information.relate_outgoing_roles_with_te_ens_to_project
Drop Function information.relate_outgoing_roles_with_te_ens_to_project;

--  fn-4 DROP function information.add_pe_it_to_project
Drop Function information.add_pe_it_to_project;

--  fn-5 DROP function information.add_pe_its_to_projects
Drop Function information.add_pe_its_to_projects;

--  fn-6 DROP function information.temporal_entity_find_or_create
Drop Function information.temporal_entity_find_or_create;

--  fn-7 refactor function information.v_statement_find_or_create
Create Or Replace Function information.v_statement_find_or_create ()
    Returns Trigger
    Language 'plpgsql'
...
Create Trigger on_insert
    Instead Of INSERT On information.v_statement For Each Row
...
-- 10 rename column war.edge (fk_role) -> (fk_statement)
Alter Table war.edge Rename Column fk_role To fk_statement;
...
From
     ...
     * TeEn is added to project, if a statement or text_property related to that PeIt or TeEn
...
From
    ...
    Where
        t1.pk_entity = t2.fk_entity And t2.is_in_project = True
    Union All
    -- domain entities of statements where info_proj_rel changed
    Select
        t1.fk_subject_info As fk_entity,
        t2.fk_project,
        t2.tmsp_last_modification,
        t2.is_in_project
    From
    ...
    Where
        t1.pk_entity = t2.fk_entity
    Union All
    -- range entities of statements where info_proj_rel changed
    Select
        t1.fk_object_info As fk_entity,
        t2.fk_project,
        t2.tmsp_last_modification,
        t2.is_in_project
    From
...
Group By
    ...
    t = clock_timestamp();
    Raise notice '#3.a query removed_statements...';

    /*
     * 3. Delete edges
     *
     * Delete the edges of statements that have a recently been removed from their project
    ...
    Where
        t1.is_in_project = False
        And t1.fk_entity = t2.pk_entity;
    -- Add the entities, related to removed statements, to to_enrich
    Insert Into to_enrich
    ...
    From
        information.persistent_item t1,
        removed_statements t2
    Where
        t1.pk_entity = t2.fk_subject_info
    Union All
    ...
    For t_row In (
        ...
        From
            removed_statements)
        Loop
            Raise NOTICE 'Edges to delete fk_project: %, fk_statement: %', t_row.fk_project, t_row.fk_statement;
        End Loop;
    -- END DEBUG
    Raise notice '...#3.a time spent=%', clock_timestamp() - t;
    t = clock_timestamp();
    Raise notice '#3.b delete edges...';
    Delete From war.edge t1 Using removed_statements t2
    Where t1.fk_project Is Not Distinct From t2.fk_project
        And t1.fk_statement = t2.fk_statement;
    -- DEBUG TIME
    Raise notice '...#3.b time spent=%', clock_timestamp() - t;
    t = clock_timestamp();
    Raise notice '#4.a query affected statements...';
     ...
     */
    Create TEMP Table updated_statements On Commit Drop As
    -- select recent info_proj_rel to statements where is_in_project = false
    Select
        array_agg( t1.fk_entity
    ) As fk_statements, t1.fk_project
From
    changed_info_proj_rel t1, information.statement t2
Where
...
Group By
    ...
    For t_row In (
        ...
        Loop
            -- DEBUG
            -- show the updated statements
            Raise NOTICE 'Updated statements fk_project: %, fk_statements: %', t_row.fk_project, t_row.fk_statements;
            Insert Into to_enrich
            Select
                fk_source,
                fk_project
            From
                war.edges__upsert_some (t_row.fk_statements, t_row.fk_project);
        End Loop;
