      ...
      FOR oldOp IN SELECT * FROM jsonb_array_elements((orig->'contents'->>'ops')::jsonb)
        ...
        IF((SELECT char_length(oldOp->>'insert') > 1)) THEN
          ...
          LOOP 
            ...
            newOps = newOps || jsonb_build_object(
              'insert', char,
              'attributes', jsonb_set(oldOp->'attributes', '{node}'::text[], latestId::text::jsonb)
            );
        ...
        ELSE 
          ...
          IF(oldOp->'attributes' ? 'node') THEN																							  
            newOps = newOps || jsonb_build_object(
              'insert', oldOp->'insert',
              'attributes', jsonb_set(oldOp->'attributes', '{node}'::text[], (oldOp->'attributes'->>'node')::int::text::jsonb)
            );
