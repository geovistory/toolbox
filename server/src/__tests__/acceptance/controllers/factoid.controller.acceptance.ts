...
describe('FactoidController', () => {
    ...
    describe('GET /get-factoid-mapping', () => {
        ...
        it('should reject the request because the query param is absent', async () => {
            const jwt = (await client.post('/login').send({ email: accountInProject.email, password: pwdIn })).body.lb4Token;
        ...
        it('Nothing should be fetched (does not exist on the table)', async () => {
            ...
            const res2 = await client.post('/set-factoid-mapping').set('Authorization', jwt).query({ pkProject: ProProjectMock.SANDBOX_PROJECT.pk_entity, pkTable: FMs.pkTable }).send({ pkTable: FMs.pkTable, mappings: [] });
            const res = await client.get(path).set('Authorization', jwt).query({ pkProject: ProProjectMock.SANDBOX_PROJECT.pk_entity, pkTable: FMs.pkTable });
        ...
        it('The Factoid mapping should correctly be fetched', async () => {
            const jwt = (await client.post('/login').send({ email: accountInProject.email, password: pwdIn })).body.lb4Token;
            const res1 = await client.post('/set-factoid-mapping').set('Authorization', jwt).query({ pkProject: ProProjectMock.SANDBOX_PROJECT.pk_entity, pkTable: FMs.pkTable }).send(FMs);
            const res = await client.get(path).set('Authorization', jwt).query({ pkProject: ProProjectMock.SANDBOX_PROJECT.pk_entity, pkTable: FMs.pkTable });
            ...
            // expect(res.body.mappings[0].properties[0].default.appellation.string).to.equal(FMs.mappings[0].properties[0].default.appellation.string)
            ...
            // expect(res.body.mappings[1].properties[0].default.resource.pk_entity).to.equal(FMs.mappings[1].properties[0].default.resource.pk_entity)
            ...
            // expect(res.body.mappings[1].properties[1].default.resource.pk_entity).to.equal(FMs.mappings[1].properties[1].default.resource.pk_entity)
        });
