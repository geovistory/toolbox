...
export async function createFactoidMapping(factoidMapping: Partial<DatFactoidMapping>) {
...
export async function createFactoidPropertyMapping(factoidPropertyMapping: Partial<DatFactoidPropertyMapping>) {
    await dealWithPkEntity(factoidPropertyMapping, 'data');
    await testdb.execute(`INSERT INTO data.factoid_role_mapping (pk_entity, fk_property, fk_column, fk_factoid_mapping) 
