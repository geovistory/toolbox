 ...
 * with isOutgoing=true and scope = {inProject:<id>}
 */
export class SqlGvFieldPageOutgoingInProject extends SqlBuilderLb4Models {
  generateFunctionSql() {
    this.sql = `
    CREATE OR REPLACE FUNCTION gv_field_page_outgoing_in_project (_project_id int, _fk_property int, _fk_subject_info int, _fk_subject_data int, _fk_subject_tables_cell bigint, _fk_subject_tables_row bigint, _limit int, _offset int, _req json)
      RETURNS TABLE (
    ...
    BEGIN
      ...
      FROM (
        ...
        WHERE
          ...
          --------------------------------------------------------------------------
        ORDER BY
          t2.ord_num_of_range ASC NULLS LAST, t1.pk_entity ASC
          --------------------------------------------------------------------------
