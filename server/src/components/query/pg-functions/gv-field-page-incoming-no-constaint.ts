 ...
 * with isOutgoing=false and scope = {noConstraint:true}
 */
export class SqlGvFieldPageIncomingNoConstraint extends SqlBuilderLb4Models {
  generateFunctionSql() {
    this.sql = `
    CREATE OR REPLACE FUNCTION gv_field_page_incoming_no_constraint (_fk_property int, _source_info_id int, _source_data_id int, _source_tables_cell_id bigint, _source_tables_row_id bigint, _limit int, _offset int, _req json)
      RETURNS TABLE (
