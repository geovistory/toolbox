 ...
 * with isOutgoing=true and scope = {noConstraint:true}
 */
export class SqlGvFieldPageOutgoingNoConstraint extends SqlBuilderLb4Models {
  generateFunctionSql() {
    this.sql = `
    CREATE OR REPLACE FUNCTION gv_field_page_outgoing_no_constraint (_fk_property int, _fk_subject_info int, _fk_subject_data int, _fk_subject_tables_cell bigint, _fk_subject_tables_row bigint, _limit int, _offset int, _req json)
      RETURNS TABLE (
