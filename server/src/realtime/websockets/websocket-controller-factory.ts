...
export interface ControllerWithMethods {[method: string]: Function};

/* eslint-disable @typescript-eslint/no-misused-promises */
export class WebSocketControllerFactory {
  private controller: ControllerWithMethods;

  constructor(
    private ctx: Context,
    private controllerClass: Constructor<ControllerWithMethods>,
  ) {
    this.ctx
      .bind('ws.controller')
  ...
  async create(socket: Socket) {
    ...
    this.controller = await this.ctx.get<ControllerWithMethods>(
      'ws.controller',
    );
