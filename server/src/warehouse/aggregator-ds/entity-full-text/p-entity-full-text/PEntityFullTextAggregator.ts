    ...
    private async loopOverEdges(
        ...
        if (edges?.length) {
            ...
            for (let i = 0; i < stop; i++) {
                ...
                let targetLabel;
                if (e.targetIsEntity) {
                    // Fetch project variant of related entity label
                    const pEntLabel = await this.providers.pEntityLabel.get({pkEntity: e.fkTarget, fkProject: fkProject})
                    targetLabel = pEntLabel?.entityLabel

                    // If needed, fetch repo variant of related entity label
                    if (!pEntLabel) {
                        const rEntLabel = await this.providers.rEntityLabel.get({pkEntity: e.fkTarget})
                        targetLabel = rEntLabel?.entityLabel
                    }
