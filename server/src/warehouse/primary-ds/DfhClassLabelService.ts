...
const updateSql = `
    ...
    WITH tw1 AS (
        ...
        FROM (
            ...
            (24781, 'zh'),
            (24901, 'zu')
        ) x(fk_entity, iso_6391 )
    )
    SELECT DISTINCT ON (dfh_pk_class,t2.fk_entity)
        dfh_pk_class "pkClass",
        t2.fk_entity "language",
        jsonb_build_object('label',t1.dfh_class_label) val
    FROM
        data_for_history.api_class t1,
        tw1 t2
    WHERE
        t1.tmsp_last_modification > $1
    AND t1.dfh_class_label_language = t2.iso_6391
`
