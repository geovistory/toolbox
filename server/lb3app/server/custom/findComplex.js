...
findComplex.prototype.operators = [
  '=',
  '!=',
  '>',
  '>=',
...
findComplex.prototype.joinTypes = [
  'INNER JOIN',
  'RIGHT JOIN',
  'LEFT JOIN',
  'FULL OUTER JOIN',
...
findComplex.prototype.castTypes = [
  'NUMERIC',
  'SMALLINT',
  'INT2',
  'INTEGER',
...
findComplex.prototype.bannedFunc = [
  'SELECT',
  'INSERT',
  'DELETE',
  'CREATE',
...
findComplex.prototype.getRelations = function(relations) {
  ...
  for (var relation in relations) {
    ...
    tmp.modelToTable =
      this.connector.schema(tmp.modelTo) +
      '.' +
      this.datasource.connector.table(tmp.modelTo);
    ...
    tmp.modelFromTable =
      this.connector.schema(tmp.modelFrom) +
      '.' +
      this.datasource.connector.table(tmp.modelFrom);
    tmp.keyFrom = relations[relation].keyFrom;
    if (
      (relations[relation].type == 'hasMany' &&
        typeof relations[relation].modelThrough == 'undefined') ||
      relations[relation].type == 'belongsTo' ||
      relations[relation].type == 'hasOne'
    ) {
    ...
    if (
      ...
      tmp.modelThroughTable =
        this.datasource.connector.schema(tmp.modelThrough) +
        '.' +
        this.datasource.connector.table(tmp.modelThrough);
...
findComplex.prototype.translateFunc = function(
  func,
  parameters,
  alias,
  subFunc = false
  ...
  switch (func) {
        ...
        if (Object.prototype.toString.call(param) === '[object Object]') {
          if (typeof param.property != 'undefined') {
            if (param.property.match(/^[A-Z_a-z][0-9A-Z_]*$/i) == null) {
              throw new Error(
                g.f(
                  '{{find()}} invalid property name %s, property must start with a letter or underscore, can only contain letters,numbers or underscore',
                  param.property
                )
              );
            }
            if (typeof param.alias != 'undefined') {
              if (param.alias.match(/^[A-Z_a-z][0-9A-Z_]*$/i) == null) {
                throw new Error(
                  g.f(
                    '{{find()}} invalid alias name %s, alias must start with a letter or underscore, can only contain letters,numbers or underscore',
                    param.alias
                  )
                );
              }
    ...
    case 'CAST':
      ...
      if (this.castTypes.indexOf(castTO) != -1) {
        ...
        throw new Error(
          g.f(
            '{{find()}} invalid cast type %s, valid cast types are: %s',
            castTO,
            this.castTypes
          )
        );
...
findComplex.prototype.createCondition = function(
  data,
  alias,
  parentParams,
  parentFunc,
    ...
    if (Object.prototype.toString.call(field) === '[object Array]') {
      ...
      if (index % 2 == moduleCondition && !isValue && !isJson) {
        ...
        if (!isValue && that.operators.indexOf(field.toUpperCase()) == -1) {
          throw new Error(
            g.f(
              '{{find()}} invalid operator name %s, valid sql operators: %s',
              field,
              that.operators.join(',')
            )
          );
        } else if (
          (field.toUpperCase() == 'AND' || field.toUpperCase() == 'OR') &&
          !isValue
        ) {
      ...
      if (field != null) {
        if (typeof field.property != 'undefined') {
          ...
          if (typeof field.alias != 'undefined') {
            ...
            sqlCondition.sql =
              sqlCondition.sql + ' ' + field.alias + '.' + field.property;
          } else {
            sqlCondition.sql =
              sqlCondition.sql + ' ' + alias + '.' + field.property;
          }
        } else {
          for (var subField in object) {
            ...
            if (func.match(/^[A-Z_a-z][0-9A-Z_]*$/i) == null) {
              throw new Error(
                g.f(
                  '{{find()}} invalid function name %s, function must start with a letter or underscore, can only contain letters,numbers or underscore',
                  func
                )
              );
            }
            ...
            parameters.forEach(function(param, index) {
              ...
              if (
                ...
                subFunc[index] = that.createCondition(
                  [subParams],
                  alias,
                  parentParams,
                  parentFunc,
...
findComplex.prototype.sqlSubquery = function(params) {
  ...
  joinType =
    params.method == 'LATERAL'
      ? params.joinType + ' LATERAL '
      : params.joinType;
  sql.sql = ' ' + joinType + ' (';
  ...
  if (params.type == 'hasManyThrough') {
    join =
      ...
      params.join_alias +
      '.' +
      params.join_foreign_key +
      ' = ' +
      params.alias +
      '.' +
      params.associationKey;
  }
  ...
  if (params.sublevels == 0) {
    groupBy =
      params.groupBy != null && typeof params.groupBy != 'undefined'
...
findComplex.prototype.translateJoin = function(
  join,
  method = 'LATERAL',
  parentJoin,
  firstPass = true
  ...
  keys.forEach(function(alias) {
    ...
    tmp.forEach(function(subJoin, index) {
      if (index == 0) {
        ...
        mainQuery = mainQuery.merge(
          that.translateJoin(
            subJoin,
            method,
            join[alias][0]['$relation'],
            false
          ),
          ''
        );
    ...
    if (
      closeJoin.parentJoin.sublevels == closeJoin.sublevel + 1 &&
      closeJoin.level != 1
    ) {
      ...
      if (method == 'LATERAL') {
        if (tmpJoin.relationData.type == 'hasManyThrough') {
          mainQuery.sql =
            mainQuery.sql +
            ' WHERE ' +
            tmpJoin.relationData.modelThrough +
            '.' +
            tmpJoin.relationData.keyTo +
            ' = ' +
            tmpParentJoin.alias +
            '.' +
            tmpJoin.relationData.keyFrom;
        } else {
          mainQuery.sql =
            mainQuery.sql +
            ' WHERE ' +
      ...
      if (closeJoin.parentJoin.groupBy != null)
        mainQuery.sql =
          mainQuery.sql + ' GROUP BY ' + closeJoin.parentJoin.groupBy;
      ...
      if (closeJoin.sublevels > 0) {
        if (method == 'LATERAL') {
          if (closeJoin.relationData.type == 'hasManyThrough') {
            mainQuery.sql =
              mainQuery.sql +
              ' WHERE ' +
              closeJoin.relationData.modelThrough +
              '.' +
              closeJoin.relationData.keyTo +
              ' = ' +
              closeJoin.parentAlias +
              '.' +
              closeJoin.relationData.keyFrom;
          } else {
            mainQuery.sql =
              mainQuery.sql +
              ' WHERE ' +
              ...
              closeJoin.relationData.keyTo +
              ' = ' +
              closeJoin.relationData.modelFrom +
              '.' +
              closeJoin.relationData.keyFrom;
          }
          if (closeJoin.where != null) {
        ...
        if (closeJoin.groupBy != null)
          mainQuery.sql = mainQuery.sql + ' GROUP BY ' + closeJoin.groupBy;
        if (closeJoin.having != null) {
          mainQuery.sql = mainQuery.sql + ' HAVING ';
          mainQuery = mainQuery.merge(closeJoin.having, '');
        }
...
findComplex.prototype.createJoinSelect = function(join, childs) {
  ...
  if (typeof join.select == 'undefined') {
    for (var property in join.relationData.modelToProperties) {
      if (join.relationData.modelToProperties[property].hidden != true)
    ...
    if (typeof join.select.include != 'undefined') {
      join.select.include.forEach(function(property) {
        if (
          typeof join.relationData.modelToProperties[property] != 'undefined'
        ) {
          if (join.relationData.modelToProperties[property].hidden != true)
            select.push(property);
        } else {
          throw new Error(
            g.f(
              "{{find()}} Property %s of model %s relation %s don't exists ",
              property,
              join.relationData.modelTo,
              join.relation
            )
          );
        }
      ...
      join.select.exclude.forEach(function(property) {
        if (
          ...
          if (removeIndex > -1) tmpSelect.splice(removeIndex, 1);
        }
      });
      select = tmpSelect;
    }
        ...
        case 'belongsTo':
          tmpSelect.push(
            "'" +
              ...
              '.' +
              alias +
              ' IS NULL THEN json_build_object() ELSE ' +
              alias +
              '.' +
              alias +
              ' END '
        ...
        case 'hasManyThrough':
          tmpSelect.push(
            "'" +
              ...
              '.' +
              alias +
              ' IS NULL THEN json_build_array() ELSE ' +
              alias +
              '.' +
              alias +
              ' END '
      ...
      case 'hasManyThrough':
        ...
        if (join.level > 0)
          sqlSelect =
            sqlSelect +
            ',' +
            join.relationData.modelThrough +
            '.' +
            join.relationData.keyTo +
            ' AS ' +
            join.relationData.keyTo;
...
findComplex.prototype.createMainSelect = function(data) {
  ...
  if (
    ...
    throw new Error(
      g.f(
        '{{find()}} select must be of type Object, %s given',
        JSON.stringify(data)
      )
    );
  } else {
    if (typeof data == 'undefined') {
      for (var property in that.modelProperties) {
        if (that.modelProperties[property].hidden != true)
          tmpSelect.push(property);
      }
    } else {
      if (typeof data.include != 'undefined') {
        data.include.forEach(function(property) {
          if (typeof that.modelProperties[property] != 'undefined') {
            ...
            throw new Error(
              g.f(
                "{{find()}} Property %s of model %s don't exists ",
                property,
                that.modelName
              )
            );
        ...
        if (typeof data.exclude != 'undefined') {
          ...
          data.exclude.forEach(function(property) {
            if (typeof that.modelProperties[property] != 'undefined') {
              removeIndex = tmpSelect.indexof(property);
              if (removeIndex > -1)
                tmpSelect = tmpSelect.splice(removeIndex, 1);
            }
          });
        }
      }
    ...
    join.orderBy.forEach(function(object) {
      for (var property in object) {
        ...
        if (that.sortOrder.indexOf(sort) == -1) {
          throw new Error(
            g.f(
              '{{find()}} Sort order %s in OrderBy for property %s in relation %s is invalid, valid values are ASC,DESC,NULLS FIRST,NULLS LAST ',
              sort,
              property,
              join.relation
            )
        ...
        if (
          ...
          throw new Error(
            g.f(
              "{{find()}} Property %s in OrderBy for model %s don't exists",
              property,
              join.relationData.modelTo
            )
          );
  ...
  switch (join.relationData.type) {
    ...
    case 'hasManyThrough':
      if (typeof join.orderBy != 'undefined') {
        join.relationData.modelToTable =
          '(select * from ' +
          join.relationData.modelToTable +
          ' AS ' +
          join.alias +
...
findComplex.prototype.createGroupBy = function(join) {
  ...
  switch (join.relationData.type) {
    ...
    case 'hasManyThrough':
      groupBy.push(
        join.relationData.modelThrough + '.' + join.relationData.keyTo
      );
      break;
  }
  if (groupBy.length != 0) {
...
findComplex.prototype.createJoins = function(
  include,
  modelName,
  firstPass = true,
  parentAlias = null,
    ...
    for (var subAlias in subData) {
      if (subAlias == '$relation') {
        ...
        if (typeof join.relationData == 'undefined') {
          throw new Error(
            g.f(
              "{{find()}} Relation %s of model %s dont't exists ",
              join.relation,
              modelName
            )
          );
        ...
        if (typeof subData[subAlias].joinType != 'undefined') {
          ...
          if (that.joinTypes.indexOf(joinType) != -1) {
            ...
            throw new Error(
              g.f(
                '{{find()}} invalid join type %s, for alias %s,relation %s',
                joinType,
                alias,
                join.relation
              )
        ...
        if (
          Object.prototype.toString.call(subData[subAlias].having) ==
          '[object Array]'
            ? subData[subAlias].having.length != 0
            : false
        ) {
        ...
        if (
          Object.prototype.toString.call(where) == '[object Array]'
            ? where.length != 0
            : false
        ) {
        ...
        if (typeof join.limit != 'undefined' && typeof join.limit != 'number') {
          throw new Error(
            g.f(
              '{{find()}} Invalid limit value %s, limit must be a number',
              join.limit
            )
          );
        }
        if (
          typeof join.offset != 'undefined' &&
          typeof join.offset != 'number'
        ) {
          throw new Error(
            g.f(
              '{{find()}} Invalid offset value %s, offset must be a number',
              join.offset
            )
          );
        }
        join.limit =
          typeof join.limit != 'undefined' ? ' LIMIT ' + join.limit : '';
        join.offset =
          typeof join.offset != 'undefined' ? ' OFFSET ' + join.offset : '';
        ...
        childData[subAlias]['$relation']['parentJoin'] =
          joinData[alias][0]['$relation'];
        if (typeof joinData[alias] == 'undefined') joinData[alias] = [];
        joinData[alias].push(
          that.createJoins(childData, modelName, false, alias, level, sublevel)
        );
        sublevel += 1;
      }
    }
...
findComplex.prototype.generateQuery = function(
  select,
  where,
  order,
  offset,
    ...
    order.forEach(function(object) {
      for (var property in object) {
        ...
        if (that.sortOrder.indexOf(sort) == -1) {
          throw new Error(
            g.f(
              '{{find()}} Sort order %s in OrderBy for property %s in model %s is invalid, valid values are ASC,DESC,NULLS FIRST,NULLS LAST ',
              sort,
              property,
              that.modelName
            )
...
findComplex.prototype.pagingQuery = function(
  pagingJoins,
  where,
  order,
  offset,
  ...
  if (
    ...
    Object.prototype.toString.call(order) == '[object Array]'
      ? order.length != 0
      : false
  ) {
    orderBy = [];
  ...
  pagingSql.sql =
    pagingSql.sql + orderBy + ' OFFSET ' + offset + ' LIMIT ' + limit + ' )';
  return pagingSql;
};
...
findComplex.prototype.find = function(filter, cb, onlyOne = false) {
  ...
  if (
    typeof filter.offset != 'undefined' &&
  ...
  if (
    typeof filter.include != 'undefined' &&
    Object.prototype.toString.call(filter.include) == '[object Object]'
  ) {
