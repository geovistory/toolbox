 ...
 * @property {Number} settings.ttl Default time to live (in seconds) for the `AccessToken` created by `PubAccount.login() / user.createAccessToken()`.
...
module.exports = function (PubAccount) {
  ...
  PubAccount.afterRemote('create', function (context, account, next) {
    ...
    PubAccount.dataSource.connector.execute(sql, params, (err) => {
      if (err)
        console.error(
          `Error while cloning sandbox project for account with id ${account.id} :`,
          err
        );
      else {
      }
   ...
   *
   */
  PubAccount.prototype.createAccessToken = function (ttl, options, cb) {
   ...
   */
  PubAccount.normalizeCredentials = function (
    credentials,
    realmRequired,
    realmDelimiter
  ) {
   ...
   *
   * ```js
   *    PubAccount.login({username: 'foo', password: 'bar'}, function (err, token) {
  ...
  PubAccount.login = function (credentials, include, fn) {
    const self = this;
    ...
    const query = self.normalizeCredentials(
      credentials,
      realmRequired,
      realmDelimiter
    );
    ...
    self.findOne({ where: query }, function (err, user) {
      ...
      function tokenHandler(err, token) {
        ...
        if (
          ...
          //     "AccessToken belongsTo PubAccount" relation
      ...
      if (err) {
        debug('An error is reported from PubAccount.findOne: %j', err);
        fn(defaultError);
      } else if (user) {
        user.hasPassword(credentials.password, function (err, isMatch) {
          if (err) {
            debug('An error is reported from PubAccount.hasPassword: %j', err);
            fn(defaultError);
          } else if (isMatch) {
            if (
              ...
              debug('PubAccount email has not been verified');
              err = new Error(
   ...
   *
   * ```js
   *    PubAccount.logout('asd0a9f8dsj9s0s3223mk', function (err) {
  ...
  PubAccount.logout = function (tokenId, fn) {
    fn = fn || utils.createPromiseCallback();
  ...
  PubAccount.observe('before delete', function (ctx, next) {
  ...
  // PubAccount.prototype.hasPassword = function(plain, fn) {
  //   fn = fn || utils.createPromiseCallback();
  ...
  // this overrides the User.prototype.hasPassword()
  // user by lb3 login function,
  // it retrieves password from credential table
  PubAccount.prototype.hasPassword = function (plain, fn) {
    const sql = `SELECT password FROM public.credential WHERE accountId = $1`;
    const params = [this.id];
    const connector = PubAccount.dataSource.connector;
    connector.execute(sql, params, (err, resultObjects) => {
      if (err) return fn(err);
      else if (resultObjects.length === 1) {
        const row = resultObjects[0];
        if (row.password && plain) {
          bcrypt.compare(plain, row.password, function (err, isMatch) {
            if (err) return fn(err);
            return fn(null, isMatch);
          });
        } else {
   ...
   * @promise
   */
  PubAccount.changePassword = function (
    userId,
    oldPassword,
    newPassword,
    options,
    ...
    // where the method is invoked from (`this` instead of `PubAccount`)
    this.findById(userId, options, (err, inst) => {
      if (err) return cb(err);

      if (!inst) {
        const err = new Error(`PubAccount ${userId} not found`);
        Object.assign(err, {
          code: 'USER_NOT_FOUND',
          statusCode: 401,
        });
   ...
   * @promise
   */
  PubAccount.prototype.changePassword = function (
    oldPassword,
    newPassword,
    options,
    cb
   ...
   * @promise
   */
  PubAccount.setPassword = function (userId, newPassword, options, cb) {
   ...
   * @promise
   */
  PubAccount.prototype.setPassword = function (newPassword, options, cb) {
    assert(!!newPassword, 'newPassword is a required argument');
  ...
  /**
   * Returns default verification options to use when calling PubAccount.prototype.verify()
   * from remote method /user/:id/verify.
   *
   * NOTE: the PubAccount.getVerifyOptions() method can also be used to ease the
   * building of identity verification options.
   *
   * ```js
   * var verifyOptions = MyPubAccount.getVerifyOptions();
   ...
   *
   * ```js
   * customPubAccountModel.getVerifyOptions = function() {
   *   const base = MyPubAccount.base.getVerifyOptions();
   ...
   * See `PubAccount.verify()` and `PubAccount.prototype.verify()` doc for params reference
   * and their default values.
   */

  PubAccount.getVerifyOptions = function () {
   ...
   *  execute the callback with the token! PubAccount saving and email sending will be
  ...
  PubAccount.prototype.verify = function (verifyOptions, options, cb) {
    if (cb === undefined && typeof options === 'function') {
      cb = options;
      options = undefined;
    }
    ...
    verifyOptions.generateVerificationToken =
      verifyOptions.generateVerificationToken ||
      PubAccount.generateVerificationToken;
    ...
    if (tokenGenerator.length == 3) {
      tokenGenerator(user, options, addTokenToPubAccountAndSave);
    } else {
      tokenGenerator(user, addTokenToPubAccountAndSave);
    }

    function addTokenToPubAccountAndSave(err, token) {
      if (err) return cb(err);
   ...
   *
   * @param {object} user The PubAccount this token is being generated for.
   * @param {object} options remote context options.
   * @param {Function} cb The generator must pass back the new token with this function call.
   */
  PubAccount.generateVerificationToken = function (user, options, cb) {
   ...
   * @promise
   */
  PubAccount.confirm = function (uid, token, redirect, fn) {
    fn = fn || utils.createPromiseCallback();
      ...
      if (err) {
        ...
        if (user && user.verificationToken === token) {
          ...
          if (user) {
            ...
            err = new Error(g.f('PubAccount not found: %s', uid));
            err.statusCode = 404;
  ...
  PubAccount.resetPassword = function (options, cb) {
    cb = cb || utils.createPromiseCallback();
    const PubAccountModel = this;
    const ttl =
      PubAccountModel.settings.resetPasswordTokenTTL || DEFAULT_RESET_PW_TTL;
    options = options || {};
    ...
    try {
      if (options.password) {
        PubAccountModel.validatePassword(options.password);
      }
    } catch (err) {
      return cb(err);
    }
    ...
    PubAccountModel.findOne({ where: where }, function (err, user) {
      if (err) {
        return cb(err);
      }
      if (!user) {
      ...
      if (
        PubAccountModel.settings.emailVerificationRequired &&
        !user.emailVerified
      ) {
      ...
      if (PubAccountModel.settings.restrictResetPasswordTokenScope) {
        const tokenData = {
          ttl: ttl,
      ...
      function onTokenCreated(err, accessToken) {
        ...
        cb();
        PubAccountModel.emit('resetPasswordRequest', {
          email: options.email,
   ...
   */
  PubAccount.hashPassword = function (plain) {
  ...
  PubAccount.validatePassword = function (plain) {
    let err;
  ...
  PubAccount._invalidateAccessTokensOfPubAccounts = function (
    userIds,
    options,
    cb
  ) {
    ...
    // between AccessToken and PubAccount
    const relatedPubAccount = AccessToken.relations.user;
    const isRelationPolymorphic =
      relatedPubAccount &&
      relatedPubAccount.polymorphic &&
      !relatedPubAccount.modelTo;
  ...
  PubAccount.setup = function () {
    // We need to call the base class's setup method
    PubAccount.base.setup.call(this);
    const PubAccountModel = this;
    ...
    PubAccountModel.setter.email = function (value) {
      if (
        !PubAccountModel.settings.caseSensitiveEmail &&
        typeof value === 'string'
      ) {
    ...
    PubAccountModel.setter.password = function (plain) {
    ...
    PubAccountModel.beforeRemote('create', function (ctx, user, next) {
    ...
    PubAccountModel.remoteMethod('getRoles', {
      accepts: {
        arg: 'id',
        type: 'number',
        required: true,
    ...
    PubAccountModel.remoteMethod('listProjects', {
      accepts: {
        arg: 'id',
        type: 'number',
        required: true,
    ...
    PubAccountModel.remoteMethod('withRolesAndProjects', {
      returns: {
        arg: 'accounts',
        type: ['PubAccount'],
        root: true,
    ...
    PubAccountModel.remoteMethod('login', {
      ...
      returns: {
        ...
        description: g.f(
          'The response body contains properties of the {{AccessToken}} created on login.\n' +
            ...
            '  - `user` - `U+007BPubAccountU+007D` - Data of the currently logged in user. ' +
            '{{(`include=user`)}}\n\n'
        ),
      },
      http: { verb: 'post' },
    ...
    PubAccountModel.remoteMethod('logout', {
    ...
    PubAccountModel.remoteMethod('prototype.verify', {
      description:
    ...
    PubAccountModel.remoteMethod('confirm', {
      description:
    ...
    PubAccountModel.remoteMethod('resetPassword', {
    ...
    PubAccountModel.remoteMethod('changePassword', {
      description: "Change a user's password.",
      accepts: [
        { arg: 'id', type: 'any', http: getPubAccountIdFromRequestContext },
        {
          arg: 'oldPassword',
          type: 'string',
          required: true,
    ...
    const setPasswordScopes = PubAccountModel.settings
      .restrictResetPasswordTokenScope
      ? ['reset-password']
      : undefined;

    PubAccountModel.remoteMethod('setPassword', {
    ...
    function getPubAccountIdFromRequestContext(ctx) {
      ...
      if (
        hasPrincipalType &&
        token.principalType !== PubAccountModel.modelName
      ) {
    ...
    PubAccountModel.afterRemote('confirm', function (ctx, inst, next) {
    ...
    assert(
      loopback.Email,
      'Email model must be defined before PubAccount model'
    );
    PubAccountModel.email = loopback.Email;

    assert(
      loopback.AccessToken,
      'AccessToken model must be defined before PubAccount model'
    );
    PubAccountModel.accessToken = loopback.AccessToken;

    PubAccountModel.validate('email', emailValidator, {
      message: g.f('Must provide a valid email'),
    });

    // Realm users validation
    if (
      PubAccountModel.settings.realmRequired &&
      PubAccountModel.settings.realmDelimiter
    ) {
      PubAccountModel.validatesUniquenessOf('email', {
        message: 'Email already exists',
        scopedTo: ['realm'],
      });
      PubAccountModel.validatesUniquenessOf('username', {
        message: 'PubAccount already exists',
        scopedTo: ['realm'],
      });
    } else {
  ...
  PubAccount.observe('access', function normalizeEmailCase(ctx, next) {
    if (
  ...
  PubAccount.observe('after save', function invalidateOtherTokens(ctx, next) {
    ...
    if (!ctx.hookState.originalPubAccountData) return next();
    ...
    const userIdsToExpire = ctx.hookState.originalPubAccountData
      .filter(function (u) {
        return (
    ...
    ctx.Model._invalidateAccessTokensOfPubAccounts(
      userIdsToExpire,
      ctx.options,
      next
    );
