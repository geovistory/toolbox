...
$BODY$;
-- 8
CREATE OR REPLACE FUNCTION war.warehouse_update_all ()
  RETURNS void
  LANGUAGE 'plpgsql'
  COST 100 VOLATILE
  AS $BODY$
...
BEGIN
  ...
  PERFORM
    war.nodes_and_edges__update_all ();
  PERFORM
    war.enriched_nodes__enrich ();
  PERFORM
...
-- 5
CREATE OR REPLACE FUNCTION war.entity_preview__update_class_labels ()
  RETURNS void
  LANGUAGE 'plpgsql'
  COST 100 VOLATILE
  AS $BODY$
...
WHERE
  ...
  AND t1.fk_project IS NOT DISTINCT FROM t2.fk_project;
  PERFORM
    war.enriched_nodes__enrich_full_text ();
  PERFORM
...
-- 4
CREATE OR REPLACE FUNCTION war.entity_preview__update_all ()
  RETURNS boolean
  LANGUAGE 'plpgsql'
  COST 100 VOLATILE
  AS $BODY$
...
-- 3
CREATE OR REPLACE FUNCTION war.entity_preview__update_modified ()
  RETURNS boolean
  LANGUAGE 'plpgsql'
  COST 100 VOLATILE
  AS $BODY$
...
-- 2
CREATE OR REPLACE FUNCTION war.entity_preview__remove_superfluous ()
  RETURNS boolean
  LANGUAGE 'plpgsql'
  COST 100 VOLATILE
  AS $BODY$
...
-- 1
CREATE OR REPLACE FUNCTION war.entity_preview__add_missing ()
  RETURNS boolean
  LANGUAGE 'plpgsql'
  COST 100 VOLATILE
  AS $BODY$
BEGIN
  ...
  WITH tw1 AS (
    ...
    FROM
      war.entity_preview)
  INSERT INTO war.entity_preview (pk_entity, fk_project, project, entity_type, fk_class, class_label, entity_label, full_text, ts_vector, type_label, fk_type, time_span, first_second, last_second)
  SELECT
    t1.pk_entity,
    t1.fk_project,
    t1.project,
