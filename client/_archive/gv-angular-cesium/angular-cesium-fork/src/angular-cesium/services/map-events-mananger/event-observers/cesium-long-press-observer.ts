...
export class CesiumLongPressObserver extends CesiumPureEventObserver {
	...
	init(): ConnectableObservable<any> {
		...
		const longPressObservable = startEventObservable.pipe(
			mergeMap((e) => observableOf(e).pipe(delay(CesiumLongPressObserver.LONG_PRESS_EVENTS_DURATION),takeUntil(stopEventObservable),)),publish(),);
