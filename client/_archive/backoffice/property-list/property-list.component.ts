...
export class PropertyListComponent extends PropertyListAPIActions implements OnInit, OnDestroy, SubstoreComponent {
  ...
  basePath = ['backoffice', 'propertiesList'];
  ...
  setTableData(items: { [key: string]: DfhProperty }) {
    this.data = this.keys.transform(items).map((item) => {
      ...
      return {
        ...item.value,
        dfh_domain_instances_max_quantifier: this.quantifierToText(item.value.domain_instances_max_quantifier),
        dfh_domain_instances_min_quantifier: this.quantifierToText(item.value.domain_instances_min_quantifier),
        dfh_range_instances_max_quantifier: this.quantifierToText(item.value.range_instances_max_quantifier),
        dfh_range_instances_min_quantifier: this.quantifierToText(item.value.range_instances_min_quantifier),
        domain_standard_label: [domainClass.dfh_identifier_in_namespace, domainClass.dfh_standard_label].join(' '),
        property_standard_label: [item.value.identifier_in_namespace, item.value.dfh_standard_label].join(' '),
  ...
  changeIdentityDefining(row) {
    this.updateProperty(new DfhProperty({
      pk_property: row.dfh_pk_property,
