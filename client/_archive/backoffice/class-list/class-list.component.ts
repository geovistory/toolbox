...
export class ClassListComponent implements OnInit, OnDestroy {
  ...
  constructor(
    ...
    this.dfhActions.label.loadOfProject(null);

    this.sysService.system_relevant_class.load();

    this.tableData$ = combineLatest(
      this.dfhService.class$.by_pk_class$.all$,
      this.dfhService.label$.by_fk_class__type$.all$,
      ...
      map(([classes, lablesByClass, sysRelClasses, pendingRows, showRemovedClasses]) => {
        ...
        return U.obj2Arr(classes)
          ...
          .map(c => {
            const systemRelevantClass = U.firstItemInIndexedGroup(sysRelClasses, c.pk_class);
            const row: TableRow = {
              dfh_pk_class: c.pk_class,
              dfh_standard_label: c.dfh_standard_label,
              profiles: c.class_profile_view,
              labels: values(lablesByClass[c.pk_class]),
              ...
              systemRelevantClass,
              pending: pendingRows[c.pk_class]
            }
            return row;
          })
  ...
  toggleSystemRelevantClassBool(row: TableRow, col: string) {
