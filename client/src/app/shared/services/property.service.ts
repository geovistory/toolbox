...
export class PropertyService {
  ...
  */
  ...
  getPropertyByPkProperty(pk): Observable<{}> {
    ...
    this.propByPkRequestCache[pk] = this.propertyApi.findById(pk).map(property => {
...
getPropertyByFkDomainClass(fk) {
  ...
  .find({ "where": { "dfh_has_domain": fk } }).map(property => {
...
getPropertyByFkRangeClass(fk) {
  ...
  .find({ "where": { "dfh_has_range": fk } }).map(property => {
