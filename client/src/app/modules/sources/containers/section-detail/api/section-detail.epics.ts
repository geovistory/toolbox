...
import { SectionDetailAPIActions, SectionDetailAPIAction } from './section-detail.actions';
...
export class SectionDetailAPIEpics {
  ...
  public createEpics(c: SectionDetailComponent): Epic {
    return combineEpics(this.createLoadSectionDetailEpic(c));
  }

  private createLoadSectionDetailEpic(c: SectionDetailComponent): Epic {
    return (action$, store) => {
      return action$.pipe(
        /**
         * Filter the actions that triggers this epic
         */
        ofType(SectionDetailAPIActions.LOAD),
