...
import { ContrVocabAPIActions } from './contr-vocab.actions';

@Injectable()
export class ContrVocabAPIEpics {
  constructor(
    private classApi: DfhClassApi,
    private actions: ContrVocabAPIActions,
  ...
  private createLoadClassEpic(subStore: ObservableStore<ClassDetail>, until$: Subject<boolean>): Epic {
    return (action$, store) => action$.pipe(
      ...
      ofType(ContrVocabAPIActions.LOAD_CLASS_DETAILS),
