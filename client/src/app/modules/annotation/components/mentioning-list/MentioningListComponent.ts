...
export class MentioningListComponent implements OnInit, AfterViewInit, OnDestroy {
   ...
   */
  @Input()
  expressionPk$: Observable<number>;
  @Input()
  expressionPortionPk$: Observable<number>;
  @Input()
  ...
  @Input()
  chunksToHighligt$: Observable<ChunksPks>;
  ...
  @Output()
  dataChange = new EventEmitter<Row[]>();
  @Output()
  rowMouseenter = new EventEmitter<Row>();
  @Output()
  rowMouseleave = new EventEmitter<Row>();
  @Output()
  rowClick = new EventEmitter<Row>();
  @ViewChild(MatSort)
  sort: MatSort;
  ...
  constructor(protected rootEpics: RootEpics, public ngRedux: NgRedux<IAppState>, private p: ActiveProjectService, private inf: InfActions, fb: FormBuilder) {
  ...
  ngOnInit() {
    this.p.pkProject$.pipe(first(), takeUntil(this.destroy$)).subscribe(pkProject => {
      if (this.listOf.type === 'digital-text') {
        ...
        const chunks$ = this.p.dat$.digital$.by_pk_entity$.key(this.listOf.pkEntity).pipe(filter(digitalVersions => !!digitalVersions && Object.keys(digitalVersions).length > 0), map(digitalVersions => values(digitalVersions)[0].pk_text), mergeMap(pkText => this.p.dat$.chunk$.by_fk_text$.key(pkText)), map(chunksByPk => values(chunksByPk)));
        const addDomain$ = chunks$.pipe(mergeMap(chunks => combineLatest(this.p.inf$.entity_association$.by_fk_data_domain$.all$, this.chunksToHighligt$)
          .pipe(map(([easByDataDomain, chunksToHi]) => chunks
            .map(chunk => {
              ...
              return partialRows;
            })), map(rowsNested => (flatten(rowsNested) as any as Row[])
        ...
        const addRange$ = addDomain$.pipe(mergeMap((rows) => {
          ...
          return combineLatestOrEmpty(pks.map(pk => this.p.streamEntityPreview(pk)))
            .pipe(filter(previews => !previews.find(p => p.loading)), map(previews => {
        ...
        const rows$ = this.p.inf$.entity_association$.by_fk_info_range$.key(this.listOf.pkEntity)
          .pipe(mergeMap((eas) => combineLatestOrEmpty(values(eas)
            .map(entityAssociation => this.p.dat$.chunk$.by_pk_entity$.key(entityAssociation.fk_data_domain)
              .pipe(filter(item => !!item), mergeMap(domainChunk => this.p.dat$.digital$.by_pk_text$.key(domainChunk.fk_text).pipe(filter(item => !!item), map(texts => latestVersion(texts)), map(digital => ({
                entityAssociation,
                domainChunk,
