...
/**
 * Cheks if the action comes from a direct child (from _children) and not
...
const ofDirectChild = (path: string[]) => (action): boolean => {
    ...
    // number of _children must one higher in actionPath than in path
    const nextLevelBool = (path.filter(s => s === '_children').length + 1) === actionPath.filter(s => s === '_children').length
...
export class DataUnitAPIEpics {
    ...
    /**
     * listenes to remove role set action and removes it from _children
     */
        ...
        return (action$, store) => {
            return action$.pipe(
                ...
                switchMap((action: RoleSetAction) => new Observable<Action>((globalStore) => {
                    ...
                    // const a = c.localStore.getState()._children
                })),
                takeUntil(c.destroy$)
            )
        }
