 ...
 */
  ...
  */
  ...
  @spyTag pipeRepoOutgoingRolesByProperty(pkProperty, pkEntity): Observable<InfRole[]> {
    return combineLatest(
      this.p.dfh$.property_view$.by_fk_property$.key(pkProperty)
        .pipe(filter(x => !!x && Object.keys(x).length > 0), map(p => values(p)[0].dfh_range_instances_max_quantifier)),
  ...
  */
  @spyTag pipeRepoIngoingRolesByProperty(pkProperty, pkEntity): Observable<InfRole[]> {
    return combineLatest(
      this.p.dfh$.property_view$.by_fk_property$.key(pkProperty)
        .pipe(filter(x => !!x && Object.keys(x).length > 0), map(p => values(p)[0].dfh_domain_instances_max_quantifier)),
