...
export class RoleSetComponent implements OnInit {
  ...
  */
  ...
  ngOnInit() {
    ...
    Observable.combineLatest(this.toggle$, this.roleStatesToCreate$).subscribe(res => {
      const toggle = res[0], roleStatesToCreate = res[1];

      if (toggle === 'expanded' && !roleStatesToCreate)
