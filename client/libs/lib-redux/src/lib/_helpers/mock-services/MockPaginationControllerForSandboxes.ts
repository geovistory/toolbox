import {
  ...
  InfTimePrimitive,
  ProInfoProjRel,
  StatementTargetDimension,
  StatementTargetEntity,
  StatementTargeTimePrimitive,
  StatementTargetLangString,
  StatementWithTarget,
...
export class MockPaginationControllerForSandboxes {
  ...
  subfieldPageControllerLoadSubfieldPages(
    gvLoadSubfieldPageReqs?: GvFieldPageReq[]
  ...
  generateData(gvLoadSubfieldPageReq?: GvFieldPageReq): GvPaginationObject {
    ...
    if (
      [1842, 1499, 1429, 133, 188, 1344, 1409].includes(
        gvLoadSubfieldPageReq?.page?.property.fkProperty
      )
    ) {
      return this.generateEmptyField(gvLoadSubfieldPageReq.page);
    }
    ...
    if (values(gvLoadSubfieldPageReq.targets)[0].appellation) {
      ...
      return this.generateDataForTimePrimitive(gvLoadSubfieldPageReq.page);
    }
    throw new Error('mock not implemented for this request');
  }

  generateEmptyField(page: GvFieldPage) {
  ...
  generateDataForLanguage(page: GvFieldPage): GvPaginationObject {
    ...
    for (let i = offset; i < offset + limit; i++) {
      ...
      const statement: OmitEntity<InfStatement> = {
        ...
        fk_subject_info: page.source.fkInfo,
        fk_property:
          DfhApiPropertyMock.EN_1112_USED_IN_LANGUAGE.dfh_pk_property,
  ...
  generateDataForEntity(req: GvFieldPageReq): GvPaginationObject {
    const page = req.page;
    const limit = page.limit;
    const offset = page.offset;
    const count = 46;
    ...
    // Do the subfields
    for (const resource of paginatedStatements
      .filter((s) => s.target.entity)
      .map((s) => s.target.entity.resource)) {
      const nestedResource =
        req.targets[resource.fk_class]?.nestedResource ?? [];
      for (const teEnSubfield of nestedResource) {
        ...
        paginationObject = mergeDeepWith(
          concat,
          paginationObject,
          subPagObject
        );
