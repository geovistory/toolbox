...
import {
  paginationObjectActions,
  schemaModifierActions,
  schemaObjectActions,
} from './data.actions';
...
export class DataEffects {
   ...
   * - else it toasts an error message
   */
  loadSchemaModifier$ = createEffect(() =>
    this.actions$.pipe(
   ...
   * - else it toasts an error message
   */
  loadSchemaModifierWithMapper$ = createEffect(() =>
    this.actions$.pipe(
   ...
   * - else it toasts an error message
   */
  loadPaginationObject$ = createEffect(() =>
    this.actions$.pipe(
      ...
      mergeMap((action) =>
        action.payload.pipe(
          ...
          mergeMap(([data, pkProject]) => {
            const pages = data.subfieldPages.map((p) => ({
              statements: p.paginatedStatements,
              count: p.count,
              page: p.page,
              pk: pkProject,
            }));
            const pagesLoadedAction = paginationActions.loadPagesSucceeded({
              pages,
            });
            return of(pagesLoadedAction, LoadingBarActions.REMOVE_JOB());
          }),
          catchError((error: HttpErrorResponse) =>
            of(...this.errorActions(error))
          ),
