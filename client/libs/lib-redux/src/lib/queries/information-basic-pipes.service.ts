...
export class InformationBasicPipesService {
  ...
  */
  pipeOutgoingStatements(pkEntity): Observable<InfStatement[]> {
    return this.infFacade.statement.getMany.by_subject$({ fk_subject_info: pkEntity })
  }
  ...
  pipeOutgoingStatementsByProperty(pkProperty, pkEntity): Observable<InfStatement[]> {
    return this.infFacade.statement.getMany.by_subject_and_property$({
  ...
  pipeInfTimePrimitive(pkEntity: number): Observable<InfTimePrimitive> {
    return this.infFacade.timePrimitive.getTimePrimitive.byPkEntity$(pkEntity)
  }
  ...
  pipeClassOfEntity(pkEntity: number): Observable<number> {
    return merge(
      this.infFacade.resource.getResource.byPkEntity$(pkEntity).pipe(
        filter(e => !!e),
        map(e => e.fk_class)
      ),
      this.infFacade.resource.getResource.byPkEntity$(pkEntity).pipe(
  ...
  pipeClassesOfPersistentItems(pkEntities: number[]): Observable<number[]> {
    return this.infFacade.resource.resourcesByPkEntity$.pipe(
      map((peIts) => {
  ...
  pipePersistentItemPksByClass(pkClass: number): Observable<number[]> {
    return this.infFacade.resource.getResource.byFkClass$(pkClass).pipe(
      map(ob => {
