export const datRoot = 'dat';
export const facetteByPk = 'by_namespace';
export const datDefinitions = {
    digital: {
        equals: (newItem, oldItem) => {
            if (!oldItem.quill_doc && newItem.quill_doc)
                return false;
            return (newItem.pk_entity === oldItem.pk_entity &&
                newItem.entity_version === oldItem.entity_version);
        },
        indexBy: {
            keyInStore: 'pk_entity__entity_version',
            indexByFn: (item) => item.pk_entity.toString() + '_' + item.entity_version.toString()
        },
        groupBy: [
            {
                keyInStore: 'pk_entity',
                groupByFn: (item) => item.pk_entity.toString()
            },
            {
                keyInStore: 'pk_text',
                groupByFn: (item) => item.pk_text.toString()
            }
        ]
    },
    chunk: {
        // facetteByPk,
        indexBy: {
            keyInStore: 'pk_entity',
            indexByFn: (item) => item.pk_entity.toString()
        },
        groupBy: [
            {
                keyInStore: 'fk_text',
                groupByFn: (item) => item.fk_text.toString()
            }
        ]
    },
    column: {
        // facetteByPk,
        indexBy: {
            keyInStore: 'pk_entity',
            indexByFn: (item) => item.pk_entity.toString()
        },
        groupBy: [
            {
                keyInStore: 'fk_digital',
                groupByFn: (item) => item.fk_digital.toString()
            }
        ]
    },
    class_column_mapping: {
        // facetteByPk,
        indexBy: {
            keyInStore: 'pk_entity',
            indexByFn: (item) => item.pk_entity.toString()
        },
        groupBy: [
            {
                keyInStore: 'fk_column',
                groupByFn: (item) => item.fk_column.toString()
            }
        ]
    },
    text_property: {
        // facetteByPk,
        indexBy: {
            keyInStore: 'pk_entity',
            indexByFn: (item) => item.pk_entity.toString()
        },
        groupBy: [
            {
                keyInStore: 'fk_entity__fk_system_type',
                groupByFn: (item) => item.fk_entity + '_' + item.fk_system_type
            }
        ]
    },
    namespace: {
        // facetteByPk,
        indexBy: {
            keyInStore: 'pk_entity',
            indexByFn: (item) => item.pk_entity.toString()
        },
        groupBy: [
            {
                keyInStore: 'fk_project',
                groupByFn: (item) => item.fk_project.toString()
            }
        ]
    }
};
//# sourceMappingURL=dat.config.js.map