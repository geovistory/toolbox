...
class ConfigurationPipesService {
    ...
    */
    pipeProfilesEnabledByProject() {
        return this.a.pkProject$.pipe(switchMap(pkProject => this.s.pro$.dfh_profile_proj_rel$.by_fk_project__enabled$
            .key(pkProject + '_true').pipe(map(projectProfileRels => values(projectProfileRels)
            .filter(rel => rel.enabled)
            .map(rel => rel.fk_profile)), map(enabled => [...enabled, DfhConfig.PK_PROFILE_GEOVISTORY_BASIC]))));
    }
    /**
    ...
    pipeFields(pkClass) {
        ...
        this.pipeProfilesEnabledByProject()).pipe(switchMap(([sourceKlass, outgoingProps, ingoingProps, sysConfig, enabledProfiles]) => {
            ...
            return combineLatest(this.pipePropertiesToSubfields(outgoingProps, true, enabledProfiles, sysConfig), this.pipePropertiesToSubfields(ingoingProps, false, enabledProfiles, sysConfig), this.pipeFieldConfigs(pkClass)).pipe(map(([subfields1, subfields2, fieldConfigs]) => {
    ...
    pipeSpecificFieldOfClass(pkClass) {
        return this.pipeFields(pkClass).pipe(map(fields => fields
    ...
    */
    ...
    pipePropertiesToSubfields(properties, isOutgoing, enabledProfiles, sysConfig) {
        return combineLatestOrEmpty(properties.map(p => {
            const o = isOutgoing;
            ...
            const sourceMinQuantity = o ?
                p.domain_instances_min_quantifier :
                p.range_instances_min_quantifier;
            return combineLatest(this.pipeClassLabel(sourceClass), this.pipeClassLabel(targetClass), this.pipeSubfieldTypeOfClass(sysConfig, targetClass, targetMaxQuantity), this.pipeFieldLabel(p.pk_property, isOutgoing ? p.has_domain : null, isOutgoing ? null : p.has_range)).pipe(map(([sourceClassLabel, targetClassLabel, listType, label]) => {
                const node = {
                    listType,
                    sourceClass,
                    sourceClassLabel,
    ...
    pipeSubfieldTypeOfClass(config, pkClass, targetMaxQuantity) {
        return this.s.dfh$.class$.by_pk_class$.key(pkClass).pipe(filter(i => !!i), map((klass) => getSubfieldType(config, klass, targetMaxQuantity)));
    }
    /**
    ...
    pipeFieldConfigs(pkClass) {
        return this.a.pkProject$.pipe(switchMap((fkProject) => {
            ...
            return combineLatest(this.s.pro$.class_field_config$.by_fk_project__fk_class$.key(activeProjectkey), this.s.pro$.class_field_config$.by_fk_project__fk_class$.key(defaultProjectkey))
                .pipe(map(([activeProjectFields, defaultProjectFields]) => {
    ...
    pipeClassLabel(pkClass) {
        return combineLatest(this.a.pkProject$, this.a.pipeActiveDefaultLanguage()).pipe(switchMap(([fkProject, language]) => this.pipeLabels({ pkClass, fkProject, language, type: 'label' })
            .pipe(map(items => {
    ...
    */
    pipeFieldLabel(fkProperty, fkPropertyDomain, fkPropertyRange) {
        ...
        return combineLatest(this.a.pkProject$, this.a.pipeActiveDefaultLanguage()).pipe(switchMap(([fkProject, language]) => this.pipeLabels({
            fkProject,
            type: 'label',
            language,
            fkProperty,
    ...
    pipeTableNameOfClass(targetClassPk) {
        return combineLatest(this.s.sys$.config$.main$, this.s.dfh$.class$.by_pk_class$.key(targetClassPk)).pipe(filter(i => !i.includes(undefined)), map(([config, klass]) => {
    ...
    pipeClassesInEntitiesOrSources() {
        return combineLatest(this.pipeClassesEnabledInEntities(), this.pipeClassesRequiredBySources()).pipe(map(([a, b]) => indexBy((x) => x.toString(), uniq([...a, ...b]))), startWith({}));
    }
    ...
    pipeClassesEnabledByProjectProfiles() {
        ...
        ]).pipe(map(([classesByPk, enabledProfiles]) => {
    ...
    */
    ...
    pipeClassesEnabledInEntities() {
        return this.a.pkProject$.pipe(switchMap(pkProject => this.s.pro$.dfh_class_proj_rel$.by_fk_project__enabled_in_entities$.key(pkProject + '_true')
            .pipe(map((rels) => values(rels).map(rel => rel.fk_class)))));
    }
    /**
    ...
    */
    pipeSelectedTeEnClassesInProject() {
        return combineLatest(this.pipeTeEnClassesEnabledInEntities(), this.pipeTeEnClassesRequiredBySources()).pipe(map(([a, b]) => indexBy((x) => x.toString(), uniq([...a, ...b]))), startWith({}));
    }
    /**
    ...
    pipeTeEnClassesEnabledInEntities() {
        return this.a.pkProject$.pipe(switchMap(pkProject => this.s.pro$.dfh_class_proj_rel$.by_fk_project__enabled_in_entities$.key(pkProject + '_true')
            .pipe(switchMap((cs) => combineLatest(values(cs).map(c => this.s.dfh$.class$.by_pk_class$.key(c.fk_class).pipe(filter(item => !!item)))).pipe(map(dfhClasses => this.filterTeEnCasses(dfhClasses)))))));
    }
    /**
    ...
    pipeTeEnClassesRequiredBySources() {
        return this.s.sys$.system_relevant_class$.by_required_by_sources$.key('true')
            .pipe(switchMap((cs) => combineLatest(values(cs).map(c => this.s.dfh$.class$.by_pk_class$.key(c.fk_class).pipe(filter(item => !!item)))).pipe(map(dfhClasses => {
    ...
    pipeTypeAndTypedClasses(enabledIn) {
        let pks$;
        const fromSources$ = this.s.sys$.system_relevant_class$.by_required_by_sources$.key('true').pipe(map(classes => values(classes).map(k => k.fk_class)));
        ...
        return combineLatest(pks$).pipe(map(arrayOfPkArrays => uniq(flatten(arrayOfPkArrays))), switchMap(pks => this.pipeTypeAndTypedClassesOfTypedClasses(pks)));
    }
    pipeTypeAndTypedClassesOfTypedClasses(pkTypedClasses) {
        return this.s.dfh$.property$.by_is_has_type_subproperty$.key('true').pipe(map((allHasTypeProps) => {
    ...
    pipeTargetClassesOfProperties(pkProperties, isOutgoing) {
        return this.s.dfh$.property$.by_pk_property$.all$.pipe(map(x => {
...
ConfigurationPipesService = tslib_1.__decorate([
    Injectable({
        providedIn: 'root'
    })
    /**
...
//   )
// }
//# sourceMappingURL=configuration-pipes.service.js.map