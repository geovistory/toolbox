...
addEventListener('message', ({ data }) => {
    if (data.task == 'csvIntoTable')
        postMessage(csvIntoTable(data.params.binaries, data.params.separator));
    if (data.task == 'parseWorkbook')
        postMessage(parseWorkbook(data.params.binaries));
    if (data.task == 'workbookIntoTable')
        postMessage(workbookIntoTable(data.params.wb, data.params.sheetName));
    if (data.task == 'sortTable')
        postMessage(sortTable(data.params.table, data.params.colNb, data.params.direction));
    if (data.task == 'filterTable')
        postMessage(filterTable(data.params.table, data.params.filters));
});
/**
 ...
 */
function csvIntoTable(binaries, separator) {
 ...
 */
function parseWorkbook(binaries) {
 ...
 */
function workbookIntoTable(wb, sheetName) {
    const table = [];
    ...
    for (let i = range.s.r; i <= range.e.r; ++i) {
        for (let j = range.s.c; j <= range.e.c; ++j) {
            ...
            if (!sheet[cellref])
                table[i - range.s.r][j - range.s.c] = '';
            else
                table[i - range.s.r][j - range.s.c] = sheet[cellref].w;
        }
    }
 ...
 */
function filterKeep(content, filter) {
    if (filter.numeric) {
        if (filter.numeric.operator == '=')
            return parseFloat(content) == filter.numeric.value;
        if (filter.numeric.operator == '>')
            return parseFloat(content) >= filter.numeric.value;
        if (filter.numeric.operator == '<')
            return parseFloat(content) <= filter.numeric.value;
    ...
    if (filter.text) {
        if (filter.text.operator == '%iLike%')
            return content.indexOf(filter.text.value) != -1;
    }
}
//# sourceMappingURL=geov.worker.js.map