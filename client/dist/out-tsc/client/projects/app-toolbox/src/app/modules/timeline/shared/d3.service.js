...
let D3Service = class D3Service {
    ...
    */
     ...
     */
    applyDraggableXAxisBehaviour(element) {
    ...
    applyRangeEmitterOnKeyDown(mouseDownElement, relativeToElement) {
        return new Observable(observer => {
   ...
   */
     ...
     */
    placeLeftOuterVisualOnXAxis(element, timeline, options) {
        this.leftBracket(element, timeline, options);
    }
    /**
    * A method to place the left inner symbol on x-axis
    */
    placeLeftInnerVisualOnXAxis(element, timeline, options) {
        this.leftBracket(element, timeline, options);
    }
    /**
      * A method to place the right inner symbol on x-axis
      */
    placeRightInnerVisualOnXAxis(element, timeline, options) {
        this.rightBracket(element, timeline, options);
    }
    /**
      * A method to place the right outer symbol on x-axis
      */
    placeRightOuterVisualOnXAxis(element, timeline, options) {
        this.rightBracket(element, timeline, options);
    }
    /**
    * A method to place the right outer symbol on x-axis
    */
    placeInnerVisualOnXAxis(element, timeline, options) {
        this.rectangle(element, timeline, options);
    }
    /**
    * A method to place the right outer symbol on x-axis
    */
    placeOuterVisualOnXAxis(element, timeline, options) {
        this.rectangle(element, timeline, options);
    }
    placeCursorOnXAxis(element, scaleX, julianSecond) {
        const d3element = d3.select(element);
        const x = scaleX.invert(julianSecond);
        d3element.attr('transform', 'translate(' + x + ', 0)');
    }
    leftBracket(element, timeline, options) {
        const d3element = d3.select(element);
    ...
    rightBracket(element, timeline, options) {
        const d3element = d3.select(element);
        const strokeWidth = timeline.options.bracketStrokeWidth;
        const halfStroke = strokeWidth / 2;
        let t = strokeWidth; //  y top
     ...
     * @param h height
     */
    rectangle(element, timeline, options) {
        const d3element = d3.select(element);
        const strokeWidth = timeline.options.bracketStrokeWidth;
    ...
    applyWrapText(element, text, width, padding) {
        const d3element = d3.select(element);
        function wrap() {
            let self = d3.select(this), textLength = self.node().getComputedTextLength(), text = self.text();
    ...
    */
    getTimeline(data, options) {
        return new Timeline(data, options, this.datePipe);
    }
    placeChartLineVisual(element, def) {
        ...
        const bi = d3.bisector(d => d.x).right;
        const bisect = (xDomain, linePoints) => {
        ...
        if (config.showCursor) {
            ...
            const started = () => {
                ...
                function dragged() {
                    let x;
                    if (d3.event.x <= config.marginLeft)
                        x = config.marginLeft;
                    else if (d3.event.x >= config.width + config.marginLeft)
                        x = config.width + config.marginLeft;
                    else
                        x = d3.event.x;
                    cursorHandle
