...
let QuillService = class QuillService {
    constructor(reg) {
        ...
        this.createTextBlot = (registry) => {
            return class TextBlot extends OriginalTextBlot {
                 ...
                 */
                retrieveUpdateType(range, newText, oldText) {
                    ...
                    if (c0 !== c1) {
                        ...
                        if (this.editorService.quillEditor.editor.delta.length() == 2)
                            return ['after', c1];
                        if (range.index > 1)
                            return ['after', c1];
                        return ['before', c0];
        ...
        this.createNodeBlot = (registry) => {
            var _a;
            const textBlot = this.textBlot;
            return _a = class NodeBlot extends Inline {
                    constructor(domNode) {
                        super(domNode);
                    ...
                    addNodeBlot(newText, updateType) {
     ...
     */
    characterizeContentChange(delta, oldDelta, latestId) {
        const latestId$ = new Subject();
        const formatText$ = new Subject();
        ...
        asapScheduler.schedule(() => {
            ...
            if (this.isInsert(delta)) {
                ...
                const addRetainOp = () => {
                    ...
                    if (delta.ops[0].insert === '' || delta.ops[0].insert === undefined)
                        delta.ops.shift();
     ...
     */
    isInsert(d) {
        return (d.ops[0] && d.ops[0].hasOwnProperty('insert') ||
     ...
     */
    endsWithRetain(d) {
        if (!d.ops[0].hasOwnProperty('retain') // first is not retain
...
QuillService = tslib_1.__decorate([
    Injectable()
], QuillService);
export { QuillService };
export function createEnterHandle() {
    ...
    return function (range, context) {
        const lineFormats = Object.keys(context.format).reduce((formats, format) => {
            if (Registry.query(format, BLOCK) &&
        ...
        Object.keys(context.format).forEach(name => {
            ...
            if (name === 'charid' || name === 'code' || name === 'link')
                return;
            this.quill.format(name, context.format[name], USER);
        });
    };
}
//# sourceMappingURL=quill.service.js.map