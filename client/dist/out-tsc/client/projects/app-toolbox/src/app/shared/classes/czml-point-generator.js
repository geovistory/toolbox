...
export class CzmlPointGenerator {
    constructor(settings) {
        ...
        this.minPx = 0;
        this.maxPx = 20;
        this.wrapPixelSizeWithZero = true;
        // Set default color
        this.d.color = {
            rgba: [20, 20, 20, 255],
    ...
    setColor(p) {
        if (p.color) {
            ...
            if (color) {
                this.d.color = Object.assign({}, this.d.color, { rgba: Cesium.Color.pack(color, []).map(n => n * 255) });
            }
        }
    }
    setOutlineColor(p) {
        if (p.color) {
            ...
            if (color) {
                ...
                this.d.outlineColor = Object.assign({}, this.d.outlineColor, { rgba: Cesium.Color.pack(color, []).map(n => n * 255) });
            }
        }
    }
    setPixelSize(p) {
     ...
     */
    getDynamicPixelSize(temporalDistribution) {
        let arr = [];
        temporalDistribution.forEach((d, i) => {
            const number = this.getPixelSizeByValue(d.items.length);
            if (number === undefined)
                throw new Error('Problem getting the pixelSize.');
...
//# sourceMappingURL=czml-point-generator.js.map