...
let ImporterComponent = class ImporterComponent {
    constructor(importTableSocket, worker, dialog, p, apiImporter, data) {
        this.importTableSocket = importTableSocket;
        this.worker = worker;
        this.dialog = dialog;
        this.p = p;
        this.apiImporter = apiImporter;
        this.data = data;
        this.destroy$ = new Subject();
        this.socketMessage$ = new BehaviorSubject('');
        // about file
        this.files = [];
        // file options CSV
        this.separators = [';', ',', '|', 'TAB'];
        this.commas = [',', '.'];
        // file options global
        this.columnsOptions = ['First row', 'No headers'];
        this.rowsNbs = ['20', '50', '100', '500', '1000'];
        this.encodings = ['UTF-8', 'UTF-16', 'ASCII', 'CYRILLIC', 'ARABIC', 'GREEK', 'HEBREW', 'CHINESE', 'KOREAN', 'BINARY', 'BASE64'];
        // informations for server
        this.namespaces = [];
        this.languages = [];
        // formControls
        this.tableNameCtrl = new FormControl('', [Validators.required]);
        this.namespaceCtrl = new FormControl('', [Validators.required]);
    ...
    selectFile(file) {
        ...
        if (this.type === 'xls' || this.encoding === 'BINARY')
            fr.readAsBinaryString(file);
        else if (this.encoding === 'BASE64')
            fr.readAsDataURL(file);
        else
            fr.readAsText(file, this.encoding.toLowerCase());
        fr.onload = () => {
            ...
            dialog.afterClosed().pipe(first()).subscribe(confirmed => {
                if (confirmed) {
                    ...
                    this.apiImporter.importTableControllerImportTable(this.namespaceCtrl.value, importTable)
                        ...
                        this.importTableSocket.on('state_' + this.fkDigital, (state) => {
                            if (this.fkDigital == state.id && state.infos != '') {
                                if (state.infos != 'inexisting')
...
ImporterComponent = tslib_1.__decorate([
    Component({
 ...
 */
function getTypeOfColumn(table, colNb, comma) {
    // We suppose that the table has the same number of col for each rows
    let isNumber = true;
