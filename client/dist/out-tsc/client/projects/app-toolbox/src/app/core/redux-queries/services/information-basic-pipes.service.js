...
class InformationBasicPipesService {
    ...
    constructor(p, s) {
         ...
         */
        this.timePrimitiveOfStatements = () => pipe(map((r) => {
            if (r.length > 1)
                return [r[0]];
            else
                return r;
        }), switchMapOr(undefined, (r) => this.pipeInfTimePrimitive(r[0].fk_object_info).pipe(switchMapOr(undefined, (infTimePrimitive) => this.p.pkProject$.pipe(switchMap((pkProject) => this.s.pro$.info_proj_rel$.by_fk_project__fk_entity$
     ...
     * Project
    *********************************************************************/
    pipeRelatedTemporalEntities(pkEntity) {
        return this.s.inf$.statement$
            .by_object$({ fk_object_info: pkEntity })
            .pipe(auditTime(1), switchMapOr([], (statements) => combineLatest(statements.map(statement => this.s.inf$.temporal_entity$.by_pk_entity_key$(statement.fk_subject_info).pipe())).pipe(map(x => x.filter((y) => !!y)))));
    }
    /**
...
tslib_1.__decorate([
    spyTag
], InformationBasicPipesService.prototype, "pipeRelatedTemporalEntities", null);
tslib_1.__decorate([
    spyTag
