...
let TypeItemComponent = class TypeItemComponent {
    constructor(p, ap, inf, i, fb) {
        this.p = p;
        this.ap = ap;
        this.inf = inf;
        this.i = i;
        ...
        this.formGroup.get('typeCtrl').valueChanges.pipe(takeUntil(this.destroy$)).subscribe(() => {
            if (!this.assigningValue)
                this.onSubmit();
        });
    }
    ngOnInit() {
        if (!this.pkEntity)
            throw new Error('You must provide a pkEntity');
        if (!this.pkProperty)
            throw new Error('You must provide a pkProperty');
        if (!this.pkTypeClass)
            throw new Error('You must provide a pkTypeClass');
        if (!this.pkTypedClass)
            throw new Error('You must provide a pkTypedClass');
        if (this.isOutgoing == undefined)
            throw new Error('You must provide a isOutgoing');
        this.ap.pkProject$.pipe(first(), takeUntil(this.destroy$)).subscribe(pkProject => {
            if (this.isOutgoing)
                ...
                this.p.inf.statement.findByParams(true, pkProject, null, this.pkEntity, null, this.pkProperty);
        });
        this.hasTypeStatement$ = this.i.pipeTypeOfEntity(this.pkEntity, this.pkProperty, this.isOutgoing);
        this.pkType$ = this.hasTypeStatement$.pipe(map(e => e ? (this.isOutgoing ? e.fk_object_info : e.fk_subject_info) : undefined));
        this.typeLabel$ = this.pkType$.pipe(switchMap(pkType => this.ap.streamEntityPreview(pkType).pipe(map(preview => preview.entity_label))));
    ...
    onSubmit() {
        combineLatest(this.hasTypeStatement$, this.ap.pkProject$).pipe(first(), takeUntil(this.destroy$)).subscribe(([statement, fk_project]) => {
            const targetEntity = this.formGroup.get('typeCtrl').value;
            // if same option is chosen or subject equals object...
            if ((statement && targetEntity == (this.isOutgoing ? statement.fk_object_info : statement.fk_subject_info)) ||
...
tslib_1.__decorate([
    Input()
], TypeItemComponent.prototype, "pkEntity", void 0);
tslib_1.__decorate([
    Input()
], TypeItemComponent.prototype, "pkProperty", void 0);
tslib_1.__decorate([
    Input()
], TypeItemComponent.prototype, "pkTypeClass", void 0);
tslib_1.__decorate([
    Input()
], TypeItemComponent.prototype, "pkTypedClass", void 0);
tslib_1.__decorate([
    Input()
], TypeItemComponent.prototype, "isOutgoing", void 0);
TypeItemComponent = tslib_1.__decorate([
    Component({
...
export { TypeItemComponent };
//# sourceMappingURL=type-item.component.js.map