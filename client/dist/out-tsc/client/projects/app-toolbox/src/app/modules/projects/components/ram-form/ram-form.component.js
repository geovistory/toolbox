 ...
 */
let RamFormComponent = class RamFormComponent {
    constructor(p, ap, ref) {
        this.p = p;
        this.ap = ap;
        this.ref = ref;
        this.destroy$ = new Subject();
        this.referenceCtrl = new FormControl();
    }
    ngOnInit() {
        ...
        this.showExactReference$ = combineLatest(this.p.ramSource$, this.p.ramProperty$).pipe(map(([s, p]) => {
            return !!s && p === DfhConfig.PROPERTY_PK_GEOVP2_MENTIONS;
        }));
        this.targetEntityPreview$ = this.p.ramTarget$.pipe(switchMap((pk) => this.ap.streamEntityPreview(pk)));
        this.source$ = this.p.ramSource$.pipe(filter(s => !!s), switchMap((source) => {
            if (source.pkEntity) {
                return this.ap.streamEntityPreview(source.pkEntity).pipe(map(ep => ({ ep })));
            }
        ...
        this.ramFormValue$ = combineLatest(this.p.ramSource$, this.p.ramProperty$, this.p.ramTarget$, this.referenceCtrl.valueChanges).pipe(map(([s, p, t, r]) => {
            // Validate source
            if (!s || (!s.chunk && !s.pkEntity))
                return;
            // Validate property
            if (!p)
                return;
    ...
    onSave() {
        combineLatest(this.p.pkProject$, this.ramFormValue$)
            ...
            .subscribe(([pkProject, val]) => {
            this.saving = true;
            if (!!val) {
                this.p.inf.statement.upsert([val], pkProject).resolved$
                    .pipe(first(res => !!res), takeUntil(this.destroy$)).subscribe(success => {
                    this.saving = false;
                    this.p.ramReset();
                });
            }
    ...
    verifyIsEntity() {
        return (entity) => !!entity.pk_entity;
    }
    ngOnDestroy() {
...
RamFormComponent = tslib_1.__decorate([
    Component({
...
export { RamFormComponent };
//# sourceMappingURL=ram-form.component.js.map