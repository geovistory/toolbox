...
import { equals, indexBy, omit, values, keys } from "ramda";
...
export class ReducerFactory {
    constructor(actionPrefix, configs) {
        ...
        this.updatingBy = (name) => 'updating_' + by(name);
        this.deletingBy = (name) => 'deleting_' + by(name);
        this.removingBy = (name) => 'removing_' + by(name);
    }
    createReducers() {
        const reducers = {};
    ...
    createModelReducers(modelName, config) {
        ...
        const reducer = (state = {}, action) => {
            ...
            if (action.type === actionPrefix + '.' + modelName + '::LOAD') {
                state = facette(action, state, (innerState) => (Object.assign({}, omit([by(config.indexBy.keyInStore)], innerState), { loading: true })));
            }
            else if (action.type === actionPrefix + '.' + modelName + '::LOAD_SUCCEEDED') {
                // If action state differs from
                state = facette(action, state, (innerState) => (Object.assign({}, this.mergeItemsInState(config, innerState, action), { loading: false })));
            }
            else if (action.type === actionPrefix + '.' + modelName + '::UPSERT') {
                state = facette(action, state, (innerState) => (Object.assign({}, innerState, { [this.updatingBy(config.indexBy.keyInStore)]: this.indexKeyObject(action, config) })));
            }
            else if (action.type === actionPrefix + '.' + modelName + '::UPSERT_SUCCEEDED') {
                state = facette(action, state, (innerState) => (Object.assign({}, this.mergeItemsInState(config, innerState, action
                // , true
                ...
                state = facette(action, state, (innerState) => (Object.assign({}, innerState, { [this.deletingBy(config.indexBy.keyInStore)]: this.indexKeyObject(action, config) })));
            }
                ...
                state = facette(action, state, (innerState) => (Object.assign({}, innerState, { [this.removingBy(config.indexBy.keyInStore)]: this.indexKeyObject(action, config) })));
            }
                ...
                state = facette(action, state, (innerState) => (Object.assign({}, innerState, omit([by(config.indexBy.keyInStore)], innerState), { loading: false })));
            }
                ...
                state = facette(action, state, (innerState) => (Object.assign({}, innerState, { [paginateBy]: Object.assign({}, innerState[paginateBy], { [key]: Object.assign({}, (innerState[paginateBy] || {})[key], { loading: Object.assign({}, ((innerState[paginateBy] || {})[key] || {}).loading, { [fromTo]: true }) }) }) })));
            }
                ...
                const fromTo = getFromTo(meta.limit, meta.offset);
                state = facette(action, state, (innerState) => (Object.assign({}, innerState, { [paginateBy]: Object.assign({}, innerState[paginateBy], { [key]: Object.assign({}, (innerState[paginateBy] || {})[key], { loading: Object.assign({}, ((innerState[paginateBy] || {})[key] || {}).loading, { [fromTo]: false }) }) }) })));
            }
                ...
                state = facette(action, state, (innerState) => (Object.assign({}, innerState, { [paginateBy]: Object.assign({}, innerState[paginateBy], { [key]: Object.assign({}, (innerState[paginateBy] || {})[key], { count: meta.count || 0, rows: Object.assign({}, ((innerState[paginateBy] || {})[key] || {}).rows, rows), loading: Object.assign({}, ((innerState[paginateBy] || {})[key] || {}).loading, { [fromTo]: false }) }) }) })));
            }
            return state;
        };
        return reducer;
...
export const by = (name) => 'by_' + name;
export const paginateName = (pagBy) => pagBy.map(p => Object.keys(p)[0]).join('__');
export const pag = (name) => 'pag_' + name;
export const paginatedBy = (name) => pag(by(name));
export const paginateKey = (pagBy) => pagBy.map(p => values(p)[0]).join('_');
