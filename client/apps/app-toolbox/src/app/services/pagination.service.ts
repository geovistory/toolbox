...
export class PaginationService {
   ...
   */
  private fieldChangeListeners = new Map<
    string,
    {
  ...
  addLoader$ = new Subject<{
    loader: Loader;
    pageIdString: string;
    takeUntil$: Observable<any>;
  }>();
  loadQueue$: Observable<
    { loader: Loader; pageIdString: string; takeUntil$: Observable<any> }[]
  >;
  ...
  constructor(
    ...
    this.fieldChangeSocket
      .fromEvent<WarFieldChange>('fieldChange')
    ...
    this.fieldChangeSocket.fromEvent('reconnect').subscribe((disconnect) => {
      this.state.pkProject$.pipe(first()).subscribe((pkProject) => {
        ...
        fieldChangeSocket.emit(
          `${FieldChangeSocket.NAMESPACE}::addToStream`,
          addMsg
        );
      });
    });
  ...
  public addPageLoader(
    ...
    const { pageIdString, fieldPage, pkProject } =
      this.parseFieldPageRequest(fieldPageReq);

    if (!this.pageLoaders.has(pageIdString)) {
      const warFieldChangeId: WarFieldChangeId =
        fieldPageToWarFieldChangeId(fieldPage);
      ...
      const fcIdString = this.addFieldChangeListener(
        warFieldChangeId,
        pageIdString
      );
      ...
      // load the page
      if (loadOnInit)
        this.loadPageAndAddSubfieldListeners(pageIdString, takeUntil$);
    } else {
    ...
    const sub = takeUntil$.subscribe(() => {
      ...
      if (loader.refCount === 1) {
        this.removeFieldLoaderAndFieldChangeListener(
          loader,
          pageIdString,
          pkProject
        );
  ...
  private decreaseFieldLoaderRefCount(
    pageIdString: string,
    loader: {
      refCount: number; // the number of references / subscribers that currently watch the page
  ...
  private removeFieldLoaderAndFieldChangeListener(
    loader: {
      refCount: number; // the number of references / subscribers that currently watch the page
      // the number of references / subscribers that currently watch the page
      until$: Subject<any>; // an observable for that emits, as soon as the page loader is not needed anymore
    ...
    const pageIds = fcListener.pageIds.filter(
      (pageId) => pageId !== pageIdString
    );
    if (pageIds.length === 0) {
      ...
      const msg: WarFieldChangeAddToStream = {
        pkProject,
        fieldIds: [fcListener.fieldChangeId],
      };
      this.fieldChangeSocket.emit(
        `${FieldChangeSocket.NAMESPACE}::removeFromStream`,
        msg
      );
    } else {
      this.fieldChangeListeners.set(loader.fieldChangeId, {
        ...fcListener,
        pageIds,
      });
    }
  ...
  private addFieldChangeListener(
  ...
  private extendFieldChangeStream(
    pkProject: number,
    warFieldChangeId: WarFieldChangeId
  ) {
    const addMsg: WarFieldChangeAddToStream = {
    ...
    this.fieldChangeSocket.emit(
      `${FieldChangeSocket.NAMESPACE}::extendStream`,
      addMsg
    );
  }
  ...
  private parseFieldPageRequest(fieldPageReq: GvFieldPageReq) {
    ...
    const pageIdString = this.getFieldPageId(fieldPage);
  ...
  /**
   * Gets the field page id string to identify a field page
   * @param page
   */
  private getFieldPageId(page: GvFieldPage): string {
    const limit = page.limit;
    const offset = page.offset;
    const fieldId = subfieldIdToString(page);
    return fieldId + '_' + limit + '_' + offset;
  }
      ...
      this.state.data
        ...
        .subscribe((res) => {
          res.subfieldPages.forEach((fieldpage) => {
            const pageIdString = this.getFieldPageId(fieldpage.page);
  ...
  private loadPageAndAddSubfieldListeners(
    pageIdString: string,
    takeUntil$: Observable<any>
  ) {
    const loader = this.pageLoaders.get(pageIdString);
  ...
  private fieldChangeToStringId(i: WarFieldChangeId): string {
    return `${i.fk_project || 0}_${i.fk_source_info || 0}_${i.fk_source_tables_cell || 0
      }_${i.fk_property || 0}_${i.is_outgoing}`;
  }
}
